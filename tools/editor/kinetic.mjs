...
export function normalizeReel(raw,{brand,domain,seconds=12,palette,captures=[]}={}){
...
export async function kineticReel({at,brand,domain,projectId,url,mode='graphic',direction='',seconds=12,palette,plan:provided,onStep=()=>{}}){
 ...
 const reply=provided?{json:provided}:await fetch(`${at}/__motioneer/ask`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({system:'You are a motion design director. Deliver a precise, original graphic film plan as JSON.',prompt:prompt+hybridPrompt,json:true,images:inventory.sheet?[inventory.sheet]:[]})}).then(r=>r.json())
 if(!reply.json)throw new Error(`Cannot direct the reel: ${reply.error||'no plan returned'}. Next: retry the brief.`)
 const plan=normalizeReel(reply.json,{brand,domain,seconds,palette,captures:inventory.captures})
 ...
 return {projectId:project.id,at,file:proved.file,url:rendered.url,proof:proved.proof,seconds,fps:60,soundtrack:'percussive',shotList:made.shots,plan,bpm:made.bpm,sourceProjectId:projectId||null,mode,sourceSheet:inventory.sheet,captures:inventory.captures.map(({data,...c})=>c),limitation:mode==='hybrid'?'Actual website captures mixed with authored graphics. Captured pixels are flattened; diagrams remain illustrative.':'Original illustrative brand graphics, not captured product UI.'}
}
