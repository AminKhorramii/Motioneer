...
export function normalizeReel(raw,{brand,domain,seconds=12,palette,captures=[],music,art,concept,bpm,brandStyle}={}){
 ...
 return {brandStyle,brand:clean(brand,24),domain:clean(domain||raw.domain,60),concept:clean(concept||raw.concept,45),motif,palette:colors,seconds,scenes,art:ART[art||raw.art]?art||raw.art:undefined,music:musicPlan({...raw.music,...(music?{profile:music}:{})},brand+' '+clean(raw.concept,45))}
}
export function reelProject(base,plan,captures=[]){
 ...
 plan.scenes.forEach((scene,i)=>{
  ...
  p.motions.push({id:motionId,subjectId,css:css+artStyles(plan.art)+(plan.brandStyle?.css||''),scope:'data-kinetic',note:scene.detail||scene.type,treatment:'bold',brief:{purpose:'entrance',intensity:'bold',duration,direction:plan.concept},duration,saved:true})
