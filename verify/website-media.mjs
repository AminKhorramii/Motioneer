...
try{
 ...
 const id=await websiteMedia({at:url,url,minimum:4})
 assert.equal(id,saved.id)
 assert.equal(saved.subjects.length,4,'two complete DOM panels, a native image and a video poster')
 ...
 assert.ok(saved.subjects.some(s=>s.name==='Issue composer'))
