...
try{
  ...
  const deep=await browser.newPage();await deep.addInitScript(()=>localStorage.setItem('motioneer-project','different-project'));await deep.goto(origin+`/__motioneer/editor/?project=${id}&view=film`);await deep.locator('.clip').first().waitFor();assert.equal(await deep.locator('.workspace-switch .active').innerText(),'Film');assert.match(await deep.locator('.project-button').innerText(),/Other tab/);await deep.close()
  const missingLink=await browser.newPage();await missingLink.goto(origin+'/__motioneer/editor/?project=missing&view=film');await missingLink.getByText('The linked film was not found in this studio.',{exact:false}).waitFor();assert.equal(await missingLink.locator('.studio').count(),0,'a broken link cannot silently open the wrong project');await missingLink.close();console.log('ok: gallery links open the requested film and reject missing projects')
