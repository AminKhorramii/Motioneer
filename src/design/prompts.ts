...
**This page is dark.** var(--bg) is a near black and var(--ink) is a near white, on every page of this wall, so build for that rather than inverting a light design at the end. Four things separate a dark page that was designed from one that was recoloured. Pure black plus pure white is a screen with the lights off, so the range you have is between them and you should use it. A large field of mid grey text is the single commonest failure, because grey on black loses the contrast that made it legible on white, so body copy stays near the top of the range and var(--dim) is for the things that genuinely recede. Accents read as light sources here rather than as ink, so one accent used once carries further than three used evenly. And a border that worked at one pixel on white disappears on black, so separate with space, weight or a change of surface before reaching for var(--line).
