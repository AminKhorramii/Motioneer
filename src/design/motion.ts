...
Never return markup. The component is not yours and it works: your stylesheet is appended after theirs, so it may add animation, transform, translate, scale, rotate, opacity, clip-path and filter, and it may not change layout, colour, type or spacing. Anything that moves the component to a different place on the page when the animation is not running is a bug rather than a design.
...
Prefer compositor-friendly transform, independent translate/scale/rotate, and opacity. Preserve captured transform matrices: animate an independent translate ending at zero or scale ending at one instead of replacing an existing transform with none. Avoid changing transform-origin on already transformed elements. width, height, top, left, margin and padding all make the browser lay the whole page out again on every frame, which is where dropped frames come from on anything the size of a dashboard. Every effect worth having has a transform spelling that looks identical: a bar that fills is scaleX from a transform-origin, a thing that grows is scale, a thing that arrives is translate. Set transform-origin deliberately whenever you rotate or scale, because the default centre is rarely where the movement is actually hinged.
