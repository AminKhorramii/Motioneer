...
export function useProject() {
  ...
  const edit=useCallback((fn:(p:Project)=>Project, checkpoint=true)=>{const p=current.current;if(!p)return;if(checkpoint){past.current=[...past.current.slice(-59),p];ahead.current=[]}publish(fn(p));dirty.current=true;setStatus('Unsaved changes');setHistory({past:past.current.length,ahead:ahead.current.length})},[publish])
  const begin=useCallback(()=>{if(!current.current)return;past.current=[...past.current.slice(-59),current.current];ahead.current=[];setHistory({past:past.current.length,ahead:0})},[])
  const undo=useCallback(()=>{const p=past.current.pop();if(!p||!current.current)return;ahead.current.push(current.current);publish({...p,revision:revision.current});dirty.current=true;setHistory({past:past.current.length,ahead:ahead.current.length})},[publish])
  const redo=useCallback(()=>{const p=ahead.current.pop();if(!p||!current.current)return;past.current.push(current.current);publish({...p,revision:revision.current});dirty.current=true;setHistory({past:past.current.length,ahead:ahead.current.length})},[publish])
  ...
  return {project,status,error,setError,projects,refresh,open,create,edit,begin,undo,redo,history,flush,current,recover}
}
